...
namespace godot
{
    ...
    {
        return t.find("function") != std::string_view::npos ||
               t.find("method") != std::string_view::npos ||
    ...
    {
        return t == "block" ||
               t.find("block") != std::string_view::npos ||
               t.find("body") != std::string_view::npos ||
    ...
    static void dump_node_types(TSNode root, const std::string &res_path)
    {
        ...
        UtilityFunctions::print("NanoCoverage: AST node types for ", String(res_path.c_str()),
                                " (", (int)types.size(), " types; first ~60 shown)");
    ...
    static std::string make_injected_line(const std::string &indent,
                                          const std::string &file_lit,
                                          uint32_t line_1_based)
    {
        return indent + "NanoCoverage.hit(\"" + file_lit + "\", " + std::to_string(line_1_based) + ")\n";
    }

    static void collect_insertions(TSNode node,
    ...
    bool Instrumenter::instrument_file_in_place(const std::filesystem::path &abs_path,
